...
static inline int lifo_policy(int policy)
{
	if(unlikely(policy == SCHED_LIFO))
		return 1;
	return 0;
}
...
static inline int task_has_lifo_policy(struct task_struct *p)
{
	return lifo_policy(p->policy);
}
...
static void __sched_fork(struct task_struct *p)
{
	...
	INIT_LIST_HEAD(&p->lifo_list_item);
	p->task_time_slice = weighted_rr_time_slice;
	//+ OS Proj2: weighted_rr_prio
...
void sched_fork(struct task_struct *p, int clone_flags)
{
	...
	if (!rt_prio(p->prio) && (p->sched_class != &lifo_sched_class))
		p->sched_class = &fair_sched_class;
...
recheck:
	...
	if (policy < 0) {
		...
		policy != SCHED_LIFO) //+ OS Proj2: weighted_rr
			return -EINVAL;
	}
...
static void init_lifo_rq(struct lifo_rq * lifo_rq)
{
	INIT_LIST_HEAD(&lifo_rq->queue);
}
